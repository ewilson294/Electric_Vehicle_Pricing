...
```{r}
#summary of FastChargeSpeed
summary(ev[,c(2,3,4,5,6,11)])
```
...
model2 <- lm(bcPower(PriceinGermany, p1$roundlam) ~ BatterySize + Acceleration + TopSpeed + Range + Efficiency + FastChargeSpeed + NumberofSeats, ev_numeric)
...
```{r}
library(leaps)
best <- regsubsets(bcPower(PriceinGermany, p1$roundlam) ~ BatterySize + Acceleration + TopSpeed + Range + Efficiency + FastChargeSpeed + NumberofSeats, data = ev_numeric, nbest = 2, method = "backward")

with(summary(best), data.frame(adjr2,bic,cp,rss, outmat))
...
```{r}
model3 <- lm(bcPower(PriceinGermany, p1$roundlam) ~ BatterySize + TopSpeed + Range + Efficiency, data = ev_numeric)

# model3 <- lm(bcPower(PriceinGermany, p1$roundlam) ~  log(TopSpeed) + I(Efficiency^-1), data = ev_numeric)
# 
# summary(powerTransform(cbind(PriceinGermany, TopSpeed, Efficiency) ~ 1, ev_numeric))
...
summary(powerTransform(cbind(PriceinGermany, BatterySize, TopSpeed, Range, Efficiency) ~ 1, ev_numeric))


model4 <- lm(bcPower(PriceinGermany, p1$roundlam) ~ log(BatterySize) +  I(TopSpeed^-1) + log(Range) + log(Efficiency), ev_numeric)
...
```{r}
a$PriceinGermany <- bcPower(ev_numeric$PriceinGermany, p1$roundlam)
a$TopSpeed <- TopSpeed^-1
a$Efficiency <- log(Efficiency)
a$FastChargeSpeed <- log(FastChargeSpeed)
...
comparison <- prediction %>% dplyr::select(PriceinGermany, pred)
...
data.frame(
  R2 = R2(prediction$pred , prediction$PriceinGermany),
  RMSE = RMSE(prediction$pred , prediction$PriceinGermany),
  MAE = MAE(prediction$pred , prediction$PriceinGermany),
...
sigma(model5)/mean(prediction$PriceinGermany)
