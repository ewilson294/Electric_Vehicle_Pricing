...
summary(powerTransform(cbind(ev$PriceinGermany, ev$Acceleration, ev$TopSpeed, ev$Range, ev$Efficiency,
...
transformed_model <- lm(1/PriceinGermany ~ sqrt(Acceleration) + (1/TopSpeed) + log(Range) +
...
transformed_model_subsets <- regsubsets(x = 1/PriceinGermany ~ sqrt(Acceleration) + (1/TopSpeed)
                             + log(Range) +log(Efficiency) + log(FastChargeSpeed)
                             + log(BatterySize), data = ev)
summary(transformed_model_subsets)
```

## Best Subset Model
```{r}
# Create model with transformed Data, without FastChargeSpeed
subset_model <- lm(1/PriceinGermany ~ sqrt(Acceleration) + (1/TopSpeed) + log(Range) +
                 log(Efficiency) + log(BatterySize), data = ev)
summary(subset_model)
plot(subset_model)
```

In this model, Normal Q-Q is close to linear, and variance is close to constant. We can reasonably make predictions with this model.
