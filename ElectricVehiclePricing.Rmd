...
ggplot(ev_numeric, aes(x = PriceinGermany)) +    
  ...
  theme_bw()
```
Now we will look at the relationship price has with acceleration, top speed, range, battery size, fast charge speed and efficiency.
```{r}
...
summary(powerTransform(cbind(PriceinGermany, Acceleration, TopSpeed, Range, Efficiency,
                             FastChargeSpeed, NumberofSeats, BatterySize) ~ 1, data = ev))
```
...
transformed_model <- lm(I(PriceinGermany^-.5) ~ sqrt(Acceleration) + I(1/TopSpeed) + log(Range) +
...
transformed_model_subsets <- regsubsets(x = I(PriceinGermany^-.5) ~ sqrt(Acceleration) + I(1/TopSpeed)
